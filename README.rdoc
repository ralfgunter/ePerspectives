...
* scan/3 - [Domain, Port, Type]: same as above (Domain is a string; port and type are integers);
...
After compiling, make sure all necessary folders/files are in place:
 ...
 server/db/ - holds the cached results
   ...
   \_ fingerprint_file
               - default storage for the fingerprints table;
               - subject to change;
               - for now, automatically created only by dets.
...
ePerspectives has been tested on Ubuntu 9.10, running Erlang/OTP R13B04.
